...
/// Font family via ttf-parser (sample.ttf generated by fonttools, OFL-clean).
#[test]
fn font_family() {
    let tmp = tempfile_dir();
    let root = tmp.join("root");
    fs::create_dir_all(&root).unwrap();
    fs::write(
        root.join("sample.ttf"),
        include_bytes!("fixtures/sample.ttf"),
    )
    .unwrap();
    ...
    let font = metas
        .iter()
        .find(|m| m.path.ends_with("sample.ttf"))
        .expect("sample.ttf");
